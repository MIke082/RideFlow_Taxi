...
export default function BoldConcept() {
  ...
  return (
    <div className="min-h-screen bg-[#FEF3C7]" dir={t.dir} style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#FEF3C7]/90 backdrop-blur-lg">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <span className="text-2xl font-black">RideFlow</span>
          <div className="flex items-center gap-3">
            <div className="flex gap-1 text-xs">
              {(["ru", "en", "he"] as Lang[]).map(l => (
                <button key={l} onClick={() => setLang(l)} className={`px-2 py-1 rounded ${lang === l ? "bg-black text-white" : "hover:bg-black/10"}`}>{l.toUpperCase()}</button>
              ))}
            </div>
            <a href="/concepts/bold/login" className="text-sm hover:underline">{t.nav.login}</a>
            <button onClick={scrollToMap} className="px-5 py-2.5 bg-black text-white text-sm font-bold rounded-full hover:bg-gray-800 hover:scale-105 transition-all">{t.nav.order}</button>
          </div>
        </div>
      </nav>
